export default function Others() {
    return (
        <div>
            <div class="text-xl font-bold">
                interests
            </div>
            <div class="flex flex-col md:flex-row mb-2.5">
                <div class="basis-1/2">
                    <ul>
                        <li class="mb-1">
                            agi/ml <br></br>[
                            <a href='https://www.coherentapp.tech/' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            coherent
                            </a>
                            /
                            <a href='https://github.com/LeonDong02/pennappsxx' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            moneymachine
                            </a>
                            /
                            <a href='https://uwflow.com/course/cs486' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            cs486
                            </a>
                            ]
                        </li>
                        <li class="mb-1">
                            security/cryptography <br></br>[
                            <a href='https://picoctf.org/' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            picoctf
                            </a>
                            /
                            <a href='https://ctf.issessions.ca/' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            issessions
                            </a>
                            /
                            <a href='https://uwflow.com/course/co487' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            co487
                            </a>
                            /
                            <a href='https://uwflow.com/course/cs458' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            cs458
                            </a>
                            ]
                        </li>
                    </ul>
                </div>
                <div class="basis-1/2">
                    <ul>
                        <li class="mb-1">
                            web3 <br></br>[
                            <a href='https://github.com/LeonDong02/habipets' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            habipets
                            </a>
                            /
                            <a href='https://twitter.com/uw_blockchain' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            uwblockchain
                            </a>
                            ]
                        </li>
                        <li class="mb-1">
                            quantum computing <br></br>[
                            <a href='https://uwflow.com/course/co481' target="_blank" rel="noreferrer" class="transition ease-in-out underline text-violet-700 hover:text-violet-900 duration-100">
                            co481
                            </a>
                            ]
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    )
}